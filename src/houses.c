...
// función para obtener la ruta del archivo de las houses según el mapa elegido
// por el usuario
char *file_location_house(char *map) {
  if (strcmp(map, "2xl_1") == 0)
    return "../maps/2xl_1/houses.txt";
  else if (strcmp(map, "lg_1") == 0)
    return "../maps/lg_1/houses.txt";
  else if (strcmp(map, "md_1") == 0)
    return "../maps/md_1/houses.txt";
  else if (strcmp(map, "xl_1") == 0)
    return "../maps/xl_1/houses.txt";
  else if (strcmp(map, "xs_1") == 0)
    return "../maps/xs_1/houses.txt";
  else if (strcmp(map, "xs_2") == 0)
    return "../maps/xs_2/houses.txt";
  return NULL;
}
...
Houses read_houses(char *map) {
  ...
  houses_list.counter = 0; // ponemos el contador en el struct para poder ir
                           // sumando 1 cada vez que añadimos una casa
  // y así es mucho mas eficiente acceder en el main solo al counter (O(1)) que
  // en el main recorrer toda la linked list que seria una complejidad O(n) de
  // muchos parametros
  ...
  while (fscanf(fp, " %[^,],%d,%lf,%lf", street, &number, &latitude,
                &longitude) == 4) {
...
// función para liberar la memoria de la linked list de casas, recorriendo la
