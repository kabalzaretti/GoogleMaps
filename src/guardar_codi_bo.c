// variables para almacenar temporalmente los datos de cada casa mientras se leen del archivo
    ...
    place_list.counter = 0; //el mismo metodo de contador para places que para houses para omptimizar el código
    ...
    fscanf(fp," %[^,],%[^,],%[^,],%lf,%lf",id ,name,category, &latitude,&longitude);
...
// función para liberar la memoria de la linked list de places, recorriendo la lista y liberando cada nodo uno por uno
